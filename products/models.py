...
class Product(models.Model):
    ...
    slug = models.SlugField(
        "Slug", max_length=100, unique=True, null=True, editable=False
    )
...
class Order(models.Model):
    ...
    )  # Store the session ID for non-logged-in users
...
class OrderAddress(models.Model):
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, unique=True, related_name="address"
    )
